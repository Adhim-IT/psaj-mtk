...
export default function MathAIPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Halo! Saya Math AI, asisten matematika Anda. Apa yang ingin Anda tanyakan tentang matematika hari ini?',
    },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to get response from Math AI');
      }
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-3xl md:text-4xl font-bold text-center mb-12">
        <span className="text-amber-500">Math AI</span> - Asisten Matematika Pintar
      </h1>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-3">
          ...
          <div className="hidden md:block bg-amber-50 p-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Tanya Apapun Tentang Matematika</h2>
            <p className="text-gray-600 mb-6">Math AI adalah asisten virtual yang dapat membantu Anda memahami konsep matematika, menyelesaikan soal, dan menjawab pertanyaan seputar matematika.</p>
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              {messages.map((message, index) => (
                <div key={index} className={`mb-4 ${message.role === 'user' ? 'flex justify-end' : 'flex justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-lg ${message.role === 'user' ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-800'}`}>
              ...
              {error && (
                <div className="flex justify-center mb-4">
                  <div className="max-w-[80%] p-3 rounded-lg bg-red-100 text-red-800">
                    <p>Error: {error}</p>
                    <p className="text-sm mt-1">Silakan coba lagi atau hubungi administrator jika masalah berlanjut.</p>
                  </div>
                </div>
              )}
